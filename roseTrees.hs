...
-- size of rose tree i.e. number of nodes
size :: Rose a -> Int
size (a :> []) = 1
size (a :> tree) = 1 + sum([size x | x <- tree])

-- number of leaves
leaves :: Rose a -> Int
leaves (a :> []) = 1
leaves (a :> tree) = sum([leaves x | x <- tree])
...
instance Functor Rose where
  fmap f (a :> tree) = (f a) :> [fmap f x | x <- tree] 
...
testSum :: Num string => Sum string
testSum = Sum 3 `mappend` Sum 4
...
class Functor f => Foldable f where
  ...
  foldMap g f = fold (fmap g f)
  
instance Foldable Rose where
  fold (a :> tree) = a `mappend` (foldr (mappend) mempty [fold x | x <- tree])
...
fsum f = unSum(foldMap (\x -> Sum x) f)
fproduct f = unProduct(foldMap (\x -> Product x) f)
