...
-- Ex. 3
-- ===================================
-- map function which applies function f to values at even positions i.e. 0,2 etc and function g to odd positions
funkyMap :: (a -> b) -> (a -> b) -> [a] -> [b]
funkyMap f g xs = [(h y) x | (x,y) <- enumerate xs] where
	h y = if y `mod` 2 == 0 then f else g

-- given list return tuple of index and value
enumerate :: [a] -> [(a, Int)]
